...
int main() {
    ...
    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, listener_socket, &accept_event) <
        0) {
    ...
    while (1) {
        ...
        for (int i = 0; i < nready; i++) {
            if (events[i].data.fd == listener_socket) {
                /*
                 * This means that the listener_socket recorded an event meaning
                 * that the listener socket might accept new peers
                 */
                 ...
                 */
                int new_peer_fd =
                    accept(listener_socket, (struct sockaddr *)&peer_addr,
                           &peer_addr_len);
                ...
                if (new_peer_fd < 0) {
                    ...
                    if (new_peer_fd >= MAXFDS) {
                        fprintf(stderr, "socket fd (%d) >= MAXFDS (%d)",
                                new_peer_fd, MAXFDS);
                        return -1;
                    }
                    fd_status_t peer_status =
                        peer_on_peer_connected(new_peer_fd);
                    ...
                    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, new_peer_fd,
                                  &event) < 0) {
                        perror("epoll_ctl EPOLL_CTL_ADD");
                        return -1;
                    }
                ...
                /*
                 * This means that another fd in the events array has recorded
                 * an event
                 */
                /*
                 * Reading
                ...
                if (events[i].events & EPOLLIN) {
                    ...
                    if (event.events == 0) {
                        ...
                        continue;
                    } else if (epoll_ctl(epoll_fd, EPOLL_CTL_MOD, event_fd,
                                         &event) < 0) {
